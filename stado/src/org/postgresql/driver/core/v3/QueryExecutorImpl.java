...
public class QueryExecutorImpl implements QueryExecutor {
    ...
    /* 
     * Try and drain buffers and unlock it
     * If we do not do this, it keeps this locked
     */ 
    public synchronized void cancelCopyFinish(CopyOperationImpl op) throws IOException       
    {              
        SQLException error = null;
        int errors = 0;
        
        if (!hasLock(op) || !copyCancelPending)
            return;
        ...
        try {
            ...
            // When there is an error it still holds the lock
            // Try and unlock anyway
            if (hasLock(op)) {
                try {
                    unlock(op);
...
//            processCopyResults(op, false); // collect any pending notifications without blocking
    ...
    private boolean copyCancelPending = false;
}
